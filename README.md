...
https://img.shields.io/badge/License-Attribution--Required-yellow.svg
https://img.shields.io/badge/C%252B%252B-11-blue.svg
https://img.shields.io/badge/Single-Header-orange.svg
...
This library is free to use for personal, educational, and commercial projects, subject to the following conditions:

Attribution Required: Any project using this library must include the following notice in its documentation, about page, or source code:

"This product includes i2wC compression library (https://github.com/Moli-i2wC/i2wC)"

Exemption List: The following individuals/organizations are exempt from the attribution requirement:

neod

Request Exemption: To request exemption from the attribution requirement, please contact: [your email]
...
.neod mod format: NeodBuilder class creates compressed mod packages with metadata, four hash-indexed tables (structures, blocks/items, entities, recipes), and O(1) lookup by key hash.
...
int main() {
    ...
    // NEW: .neod mod packaging
    i2wC::NeodBuilder neod;
    neod.setMagic(0x4E454F44);                 // "NEOD"
    neod.setCompressionLevel(6);
    neod.setMeta("Name", "My Awesome Mod");
    neod.setMeta("Version", 0x00010000U);     // 1.0.0
    neod.setMeta("Author", "Modder");
    neod.setMeta("Preview", previewImageData); // PNG binary

    // Add entries to tables (hash = sum of characters in key)
    neod.addStructureEntry("ruins_large", structureData);
    neod.addBlockItemEntry("mod:magic_sword", itemData);
    neod.addEntityEntry("fire_dragon", entityData);
    neod.addRecipeEntry("super_helmet", recipeData);

    std::vector<uint8_t> neodFile;
    neod.serialize(neodFile);                // Compressed .neod binary
    // Save to file or embed in game

    // Later, load a .neod file
    i2wC::NeodBuilder loader;
    loader.deserialize(neodFile);
    std::vector<uint8_t> retrieved;
    loader.getMeta("Name", retrieved);        // "My Awesome Mod"
    loader.getStructureTable().getEntry(hash, structureData);
    return 0;
}
API 概览 / API Overview
...
NeodBuilder	.neod mod 包构建器：元数据键值对 + 四个哈希索引表（结构/物品/实体/合成），自动压缩 / .neod mod package builder: metadata KV pairs + four hash-indexed tables, auto-compressed
NeodHashTable	基于字符串字符和哈希的哈希表，O(1) 存取 / Hash table using sum-of-chars hash, O(1) access
功能裁剪宏 / Feature Control Macros
...
署名要求：任何使用本库的项目，必须在文档、关于页面或源代码中保留以下声明：
...
申请豁免：如需申请豁免署名要求，请联系：[你的邮箱]

免责声明：本软件按"原样"提供，不提供任何明示或暗示的保证，包括但不限于适销性、特定用途适用性和非侵权性的保证。
...
.neod mod 格式：NeodBuilder 类可创建压缩的 mod 包，包含元数据和四个哈希索引表（结构、方块/物品、生物、合成），支持按键哈希 O(1) 查找。
...
int main() {
    ...
    neod.setMagic(0x4E454F44);                 // "NEOD"
    neod.setCompressionLevel(6);
    neod.setMeta("Name", "我的超赞 Mod");
    neod.setMeta("Version", 0x00010000U);     // 1.0.0
    neod.setMeta("Author", "模组作者");
    neod.setMeta("Preview", previewImageData); // PNG 二进制

    // 向表中添加条目（哈希 = 字符串字符和）
    neod.addStructureEntry("巨大遗迹", structureData);
    neod.addBlockItemEntry("mod:魔法剑", itemData);
    neod.addEntityEntry("火龙", entityData);
    neod.addRecipeEntry("超级头盔", recipeData);

    std::vector<uint8_t> neodFile;
    neod.serialize(neodFile);                // 压缩后的 .neod 二进制
    // 保存到文件或嵌入游戏
    ...
    loader.deserialize(neodFile);
    std::vector<uint8_t> retrieved;
    loader.getMeta("Name", retrieved);        // "我的超赞 Mod"
    loader.getStructureTable().getEntry(hash, structureData);
    return 0;
}
API 概览
...
NeodBuilder	.neod mod 包构建器：元数据键值对 + 四个哈希索引表（结构/物品/实体/合成），自动压缩
NeodHashTable	基于字符串字符和哈希的哈希表，O(1) 存取
功能裁剪宏
在包含 i2wC.h 之前定义以下宏，可裁剪不需要的功能，减小最终二进制体积。
